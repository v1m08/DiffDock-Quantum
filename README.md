...
Quantum DiffDock is a science-fair research fork of
[DiffDock](https://github.com/gcorso/DiffDock) exploring a more physically
informed, quantum-inspired pipeline for structure-based virtual screening.
It keeps DiffDock's diffusion-based pose generation and adds an experimental
post-processing workflow that compares ligand protonation states with
AutoDock Vina binding energies.
...
complement geometric RMSD-based docking metrics. The science-fair evaluation
...
geometry. These results are a proof of concept, not a complete benchmark:
compute and time constraints limited testing, and the optimizer was applied
after inference rather than trained jointly with DiffDock.
...
history, model code, license, and citations. The original project documentation
continues below.
